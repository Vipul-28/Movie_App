...
class MovieRepositoryImpl extends MovieRepository{
  ...
  @override
  Future<Either<AppError, List<MovieEntity>>> getAllMovies() async {
    try {
  ...
  Future<Either<AppError,List<CastModel>>> getCastCrew(int id) async {
    try {
      final movies = await remoteDataSource.getCastCrew(id);
      return Right(movies);
    } on SocketException {
      return Left(AppError(AppErrorType.network));
    } on Exception {
  ...
  Future<Either<AppError,List<VideoModel>>> getVideos(int id) async {
    try {
      final movies = await remoteDataSource.getVideos(id);
      return Right(movies);
    } on SocketException {
      return Left(AppError(AppErrorType.network));
    } on Exception {
